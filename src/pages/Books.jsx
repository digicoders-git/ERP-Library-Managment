...
const Books = () => {
  ...
  const handleSubmitAdd = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await bookAPI.create(formData);
      if (response.data.success) {
  ...
  const handleSubmitEdit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await bookAPI.update(editingBook._id, formData);
      if (response.data.success) {
  ...
  return (
    <div className="p-6">
              ...
              {currentBooks.length > 0 ? currentBooks.map((book) => (
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-8 text-center text-gray-500">
                    {loading ? 'Loading books...' : 'No books found. Add books to get started.'}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {showHistory && selectedBook && (
        <div className="fixed inset-0 backdrop-blur-md bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowHistory(false)}>
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-gray-200">
      ...
      {/* Add Book Modal */}
      {showAddModal && (
        <div className="fixed inset-0 backdrop-blur-md bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowAddModal(false)}>
